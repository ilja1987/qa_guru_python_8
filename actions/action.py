...
def open_page():
    ...
    ads.with_(timeout=10).should(have.size_greater_than_or_equal(3)).perform(command.js.remove)
    google_uploadeds = browser.all('[id^=google_uploaded]')
    if len(google_uploadeds) > 0:
        google_uploadeds.with_(timeout=10).should(have.size_greater_than_or_equal(1)).perform(command.js.remove)
